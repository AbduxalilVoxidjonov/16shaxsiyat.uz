...
| `SCHOOL_CODE_INVALID` | 404 | `POST /api/public/schools/resolve-code`: maktab kodi topilmadi — format xato, mavjud emas, nofaol, o'chirilgan, ommaviy makon: HAMMASI bitta kod (enumeration'ga qarshi; 2026-09-07) |
...
| `REGISTRATION_FIELD_REQUIRED_FOR_BATTERY` | 400 | Shaxsiyat batareyasi bor dasturda `RegistrationFields.BirthDate`/`Grade`ni `Required`dan boshqasiga qo'yib/nashr qilib bo'lmaydi (`gender` bundan mustasno) (P52 kengaytmasi, 2026-09-11) |
...
| 2026-09-07 | Maktab kodi (`School.EntryCode`, 8 belgi, `XXXX-XXXX`) — `/kirish` da "Maktab uchun" yo'li; `resolve-code` kodni `{slug, accessToken}` ga aylantiradi, keyin MAVJUD `/t/{slug}?k=` oqimi | Egasi | O'quvchi akkaunt ochmasdan, havolasiz, faqat kod bilan kira olsin. Yangi oqim yozilmadi — kod mavjud oqimga "eshik". `AccessCode` (sinf kodi) semantikasi o'zgarmadi. Generatsiya Infrastructure'da (`RandomNumberGenerator`), domen faqat format. Rad holatlari bitta `404 SCHOOL_CODE_INVALID`; IP bo'yicha 10/5 daqiqa; muvaffaqiyatsiz urinish audit (`SchoolCode.ResolveFailed`, kod qiymatisiz) |
...
| 2026-09-11 | **Ro'yxatdan o'tish dasturga biriktiriladi** (`AssessmentProgram.RegistrationMode`: `Full`/`None`). `None` dasturda registratsiya ekrani UMUMAN ko'rsatilmaydi, `Student` ANONIM yaratiladi (`Student.CreateAnonymous`, `IsAnonymous`, `BirthDate`/`Phone` nullable). Qat'iy invariant: shaxsiyat batareyasi bor dasturda `RegistrationMode` DOIM `Full` (`REGISTRATION_REQUIRED_FOR_BATTERY`, 400) | **Loyiha egasi** (PM so'radi, egasi tanladi) | Ro'yxatdan o'tish anketasi (F.I.Sh./tug'ilgan sana/jins/sinf/telefon) va so'rovnomaning `Q1_1–Q1_4` savollari BIR XIL ma'lumotni ikki marta so'rayotgan edi. Qabul qilingan kamchilik: anonim sessiyada admin ro'yxatida ism ko'rinmaydi va BR-1 (90 kunlik takror topshirish) tekshiruvi ishlamaydi — egasi buni bilib turib tanladi. Faqat `POST /api/public/sessions` (maktab oqimi) qamrovida; `StartPublicSessionCommand` (Telegram/ommaviy makon) TEGILMADI |
| 2026-09-11 | **Registratsiya maydonlari HAR DASTURDA alohida sozlanadi** — yangi qiymat obyekti `AssessmentProgram.RegistrationFields` (`Hidden`/`Optional`/`Required` har maydon uchun, `docs/18` §9.5). `fullName` sozlamada yo'q (har doim majburiy `Full` rejimida). Batareya invarianti kengaydi: `birthDate`/`grade` DOIM `Required` (`REGISTRATION_FIELD_REQUIRED_FOR_BATTERY`, 400), `gender` esa erkin. `Student.Create`dagi "anonim bo'lmagan o'quvchida `birthDate`/`phone` DOIM to'ldirilgan" invarianti (`STUDENT_IDENTITY_REQUIRED`) OLIB TASHLANDI — endi bu ikkalasi ham maydon sozlamasiga qarab `null` bo'lishi mumkin. BR-1 takrorlanish tekshiruvi `birthDate` yo'q bo'lganda UMUMAN bajarilmaydi (har doim yangi `Student`) | **Loyiha egasi** (PM ikki variant taklif qildi — umumiy sozlama yoki har dasturda alohida — egasi ikkinchisini tanladi) | Ikkilik (`Full`/`None`) rejimi "hammasi yoki hech narsa" edi: ota-ona telefoni yoki email so'ralmasligi kerak bo'lgan dasturlarda ham F.I.Sh./tug'ilgan sana/telefon bilan birga ularni ham so'rashga majbur qilardi |
