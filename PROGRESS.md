...
**Oxirgi yangilanish:** 2026-09-12 · **Joriy bosqich:** B8 (Anketa konstruktori — tarmoqlanish) · **Joriy vazifa:** P55 (GLOBAL ro'yxatdan o'tish formasi sozlamasi, 1-to'lqin) yakunlandi; sessiya oqimlariga ulash (2-to'lqin) kutilmoqda
...
| P53 | Egasi topgan uch xato/talab: arxivlangan dastur testlari, matnga yozib bo'lmasligi, registratsiyani dasturga biriktirish | backend + frontend | ✅ | — | **Bloklovchi:** matn savoliga bitta harfdan keyin yozib bo'lmasdi (fokus effekti beqaror bog'liqlikda) — real brauzerda ikki tomonlama tasdiqlandi, vitest+E2E mutatsiya sinovi bilan qulflandi · kirish ekrani `tests[]` endi dasturga bog'langan (`ProgramTestCatalog` yagona manba) · `RegistrationMode` (`Full`/`None`) + anonim `Student`, batareyali dasturda `None` taqiqlangan · `hasPersonalityBattery` admin DTO'siga qo'shilib frontend qattiq kod ro'yxati olib tashlandi · backend **1593**, frontend **934** test · **jonli muhitga joylashtirildi va tekshirildi (2026-09-11)** |
| P54 | Savol o'chirish xatosi + ro'yxatdan o'tish formasi sozlamasi | backend + frontend | ✅ | — | **Egasi topdi:** savol o'chirilmasdi va `500` berardi — sabab: javobi bor savolni DB o'chirishga qo'ymaydi, FK buzilishi esa hech qayerda tarjima qilinmagan edi. Endi `QUESTION_IN_USE` (409) + tugma oldindan qulflanadi + tezkor "Faol emas" almashtirgichi · shart tayangan savolni o'chirish ham bloklanadi (`QUESTION_REFERENCED_BY_VISIBILITY`) — ilgari anketa JIMGINA buzilardi · `RegistrationFields` har dasturda: Yashirin/Ixtiyoriy/Majburiy · **eski nomuvofiqlik yopildi:** forma jinsni majburiy qilardi, server tekshirmasdi · backend **1640**, frontend **962** test · jonli muhitga joylashtirildi (2026-09-11) |
...
| P55 | GLOBAL ro'yxatdan o'tish formasi sozlamasi — 1-to'lqin (domen + saqlash + admin API + hujjat) | backend-dotnet | ✅ | — | **Egasining talabi:** kecha qo'shilgan `AssessmentProgram.RegistrationFields` (har dasturda alohida) bir kundan keyin GLOBAL sozlamaga almashtirildi (`RegistrationFormSettings`, singleton jadval `registration_form_settings`) — ikki joyda bir xil sozlama takroriy muammo naqshi bo'lgani uchun · yangi domen: `RegistrationFormDefinition`/`RegistrationCoreFields`/`RegistrationCustomField` (`Domain/Settings/`), 6 ta yangi xato kodi (`REGISTRATION_FORM_*` + qayta ishlatilgan `INPUT_PATTERN_INVALID`) · `GET`/`PUT /api/admin/settings/registration-form` (`SettingsController`, faqat superadmin) · migratsiya `AddRegistrationFormSettings` (faqat qo'shish) · **ATAYLAB QILINMADI (keyingi to'lqin):** sessiya oqimlariga (`StartSession`/`GetSchoolInfo`) ulash va frontend — eski `AssessmentProgram.RegistrationFields` HALI ishlaydi, xossa/ustun o'lik hisoblanadi lekin o'chirilmadi (risklar jadvali) · backend **1679 test** (28 yangi domen + 7 yangi integratsiya), `dotnet build` 0 xato/0 ogohlantirish, `has-pending-model-changes` bo'sh |
| P56 | Global ro'yxatdan o'tish formasi konstruktori + tarixdagi javoblarni ko'rish | backend + frontend | ✅ | — | Sozlamalarda forma konstruktori: maydon **matnini** tahrirlash, Yashirin/Ixtiyoriy/Majburiy, **o'z maydonlarini** qo'shish (`profile_extra` jsonb) · ikkala oqim ham global sozlamadan (Telegram'da "bir marta so'ralishi" saqlangan) · dastur formasidagi per-program jadval OLIB TASHLANDI (yagona manba) · tarix qatorlari sessiya sahifasiga olib boradi, u yerda savolma-savol javoblar (so'rovnomada Likert ustunlarisiz, ko'p tanlovda variant NOMLARI) · `AnswersSection` `widgets/` ga ko'chirildi (features bir-birini import qilmasligi qoidasi) · tugash sahifasida «Bosh sahifaga qaytish»/«Kabinetga qaytish» · backend **1698**, frontend **1054** test · jonli muhitga joylashtirildi (2026-09-12) |
...
|-------------|--------|------|
| `assessment_programs.registration_fields` ustuni ENDI O'LIK (2026-09-11/12): ro'yxatdan o'tish sozlamasi GLOBAL (`registration_form_settings`, `RegistrationFormSettings`) ga ko'chirildi, lekin `StartSessionCommandHandler`/`GetSchoolInfoQueryHandler`/admin `CreateProgram`/`UpdateProgram` HALI shu eski ustunni o'qiydi/yozadi (1-to'lqin doirasida sessiya oqimlariga ataylab tegilmadi) | Ikkita parallel "ro'yxatdan o'tish sozlamasi" manbai bor — biri ishlaydi (eski, dasturga bog'liq), biri hali hech qanday oqimga ulanmagan (yangi, global). Superadmin yangi "Sozlamalar" sahifasida narsa o'zgartirsa, o'quvchi ko'radigan forma HALI eskisiga tayanadi | **Keyingi to'lqin (session oqimlariga ulash):** `StartSessionCommandHandler`/`GetSchoolInfoQueryHandler` `RegistrationFormSettings`ga o'tkaziladi, `AssessmentProgram.RegistrationFields` xossasi/ustuni va admin `Create`/`UpdateProgram`dagi `registrationFields` parametri OLIB TASHLANADI, ustun alohida EF Core migratsiyasida (`docs/06` 7-qoidasi — destruktiv o'zgarish ikki bosqichda) o'chiriladi. Tafsilot: `docs/18` §9.6.2 |
...
| ~~"Dasturda shaxsiyat batareyasi bormi" frontend'da `"MBTI16"` kodini qidirish bilan aniqlanadi~~ — **to'liq yopildi (P52, 2026-09-11)**: backend qismi avval (`hasPersonalityBattery` bayrog'i `AdminProgramListItemDto`/`AdminProgramDetailDto`da), endi frontend ham (`ProgramFormDialog` shu bayroqqa o'tdi; `programComputations.ts`dagi `hasPersonalityBattery()`/`types.ts`dagi `PERSONALITY_BATTERY_TEST_CODES` butunlay olib tashlandi) | — | — |
