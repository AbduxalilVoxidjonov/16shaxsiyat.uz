...
/**
 * `POST /api/me/sessions` domen xatolari (`docs/07` §5.4 jadvali) → foydalanuvchiga
 * tushunarli matn. Forma (`PublicRegistrationForm`) va "tayyor" karta (`PublicRegistrationPage`
 * `ready` holati) BIR XIL xaritadan foydalanadi — ikki nusxa bo'lsa vaqt o'tib ajralib ketardi.
 */
export function mapStartSessionErrorCode(error: AppError, t: TFunction): string {
  ...
  return codeMessages[error.code] ?? error.message ?? t('account.register.errors.generic');
}
