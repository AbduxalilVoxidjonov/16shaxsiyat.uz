...
function allTestsCompletedState() {
  return {
    ...
    tests: [
      ...
      { code: 'RIASEC', status: 'Completed', answered: 48, total: 48, order: 3 },
      { code: 'ACTIVITY', status: 'Completed', answered: 32, total: 32, order: 4 },
    ],
    progressPercent: 100,
  };
}
...
function mockFetch({
  ...
  completeSession?: unknown;
} = {}) {
  const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
    const url = String(input);
    const method = init?.method ?? 'GET';
    if (url.includes('/sessions/me')) {
      return Promise.resolve(jsonResponse(sessionState));
    }
    if (url.includes('/sessions/complete') && method === 'POST') {
      return Promise.resolve(jsonResponse(completeSession));
    }
...
function renderPage(initialPath = '/t/demo-school/finish') {
